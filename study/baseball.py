...
class Record:
    ...
    def batter_record(self, hit, homerun):
        ...
        self.atbat += 1
        self.avg = self.hit / self.atbat # 타율 = hit/타수
...
class Game:
        ...
        if Game.OUT_CNT < 3:
            ...
            while True:
                ...
                if hit_cnt == 0:  # strike !!!
                    ...
                    Game.STRIKE_CNT = 0
                    if hit_cnt != 4:
                        print('== ▣ {}루타!!!\n'.format(hit_cnt)) # 1루타, 2루타, 3루타
                    else:
                        print('== ▣ 홈런!!!\n') # 4개 맞으면 홈런
                    self.advance_setting(hit_cnt)
                    break

            player.hit_and_run(1 if hit_cnt > 0 else 0, 1 if hit_cnt == 4 else 0) # hit_cnt(1,2,3,4) 이면 안타(hit)이 1 올라감
    ...
    def hit_judgment(self, random_ball, hit_numbers): # random_ball, hit_number 를 고정이 아니라 가변으로 하려면 어떻게??
                                                      # random_ball, hit_number / random_balls(랜덤 수 4개 set), hit_numbers 다른 것이구나!
        cnt = 0
        for hit_number in hit_numbers:
            if hit_number in random_ball: # 여기서 cnt를 반환한다. / 왜 hit_cnt 가 아니라 cnt??????????
                cnt += 1
        #print(cnt) 아마도 0 , 1, 2, 3,4  을 반환하는 듯. 2, 3, 4은 본 적 없지만.
        return cnt
########################## 타율 - 해리 ###################################

    def avg_number(self, avg): # 타율에 따라 랜덤수 조정 / 타율 1.0 ~ 0.5 : 랜덤 3, 타율 0.5 ~ 0.0 랜덤 5, 디폴트: 랜덤 4
        if self.atbat >=2 and self.avg > 0.5:
            avg_n = 3
        elif self.atbat >=2 and self.avg <= 0.5:
            avg_n = 5
        else:
            avg_n = 4

    # 선수가 입력한 숫자 확인 ## 여기서 타율 적용해서 len(hit_numbers) 바꿔줘야 하고
    def hit_number_check(self, hit_numbers):
        if len(hit_numbers) == self.avg_n: # 이게 가변이려면 4를 n 변수로 선언하고
            for hit_number in hit_numbers:
    ...
    # 랜덤으로 숫자 생성(1~40) ## 여기서도 똑같이 바꿔줘야 하는데!!!! len(random_balls)
    def throws_numbers(self):
        random_balls = set()
        while True:
            random_balls.add(random.randint(1, 40))  # 1 ~ 20 중에 랜덤 수를 출력
            if len(random_balls) == self.avg_n:  # 생성된 ball 이 n개 이면(set 객체라 중복 불가)
                return random_balls
