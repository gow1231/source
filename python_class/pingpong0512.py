...
class Ball:
    ...
    def keystate(self, movement):
        ...
        return (paddle_pos[0], (ball_pos[0], ball_pos[1]), (self.x, self.y), movement)  # 206.0 (419.0, 50.0) (-3, 3)

    def add(self, key):
        self.values[key] = 0 # 게임이 진행중일때는 0이니까.
    ...
    def randomChoice(self):
        rand = random.choice([0,1]) # 0,1 중에 하나를 랜덤으로 골라
        key = self.keystate(rand) # rand 를 넣어서 학습데이터를 찾아볼 것임.
        if key not in self.values: # 만약 없으면 add
            self.add(key)
        return rand # 0 or 1이 출력됨.
...
while 1:
    ...
    if ball.hit_bottom == False:
        ...
        starts = [0,1] # 실제로 여기 넣는 데이터는 0 or 1 PADDLE_MOVE[0] or [1] 에서 -2 or 2로 가져옴.
